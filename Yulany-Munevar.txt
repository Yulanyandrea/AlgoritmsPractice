...
function sortPaddockTypeByTotalArea() {
  // CODE HERE
   const array =[]
   paddocks.map(crop => {
    const nameObj = farms.find(obj => obj.id === crop.farmId);
    const name = nameObj?.name
    const hectares = crop.area;
    if (name in array) {
      array[name] += hectares;
    } else {
      array[name] = hectares;
...
function sortFarmManagerByAdminArea() {
  // CODE HERE
  const array =[]
  paddocks.forEach(crop => {
    const nameObj = paddockManagers.find(obj => obj.id === crop.paddockManagerId);
    const name = nameObj.name;
    const hectares = crop.area;
...
function farmManagerNames() {
  ...
  const cropName = paddocks.reduce((acc, curr) => {
    const farmName = farms.find(farm => farm.id === curr.farmId)
    const fieldName = ` ${farmName.name}`;
    const admin = paddockManagers.find(name => name.id === curr.farmId)
    ...
    if(!acc[fieldName].includes(`${admin.name}`)){
      acc[fieldName].push(`${admin.name}`);
      acc[fieldName].sort();
...
function biggestCherriesManagers(objs,objsname,objfarm) {
  // CODE HERE
  let array = []
  const farmResult = farms.map(farm =>{
    if(farm.name === 'FORESTAL Y AGRICOLA LO ENCINA'){
      paddocks.map(area =>{
        if( area.area > 1000 && area.paddockTypeId === 3){
          paddockManagers.map(name =>{
            if(name.name === 'CARLOS PEREZ GONZALEZ'){
              array.push(name.name, area.area)
...
function farmManagerPaddocks() {
  // CODE HERE
  const admin = {}
  paddockManagers.forEach(field => {
    if (!admin[field.name]) {
      admin[field.name] = [];
    }
    farms.forEach(nameFarm =>{
      if(nameFarm.id === field.id){
        admin[field.name].push(nameFarm.name);
...
function paddocksManagers() {
  ...
  const cropYearMap = paddocks.reduce((acc, curr) => {
    const cropAdmin = paddockManagers.find(data => data.id === curr.farmId)
    const cropType = paddockType.find(type => type.id === curr.farmId);
    const cropYear = curr.harvestYear;
    const key = `${cropType?.name}-${cropYear}`;
    if (!acc[key]) {
      acc[key] = {};
    }
    acc[key][cropAdmin.id] = cropAdmin.name;
    return acc;
  }, {});
...
function newManagerRanking() {
  ...
  paddockManagers.push({ id: 7, taxNumber: '123456789', name: 'ANDREA CASTRO' });
